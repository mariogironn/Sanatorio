...
<head>
  ...
  <style>
    ...
    .main-page-title i {
      ...
      font-size: 2rem;
      color: #6f42c1;
      background: linear-gradient(135deg, #6f42c1, #8367d8);
      -webkit-background-clip: text;
      -webkit-text-fill-color: transparent;
      background-clip: text;
    }
    
    /* ========== KPIS ESTILO SEGUNDA IMAGEN ========== */
    .kpi-container {
      margin-bottom: 2rem;
    }
    ...
    /* Colores vibrantes para cada KPI */
    .kpi-total { 
    ...
    /* Header de la página del catálogo */
    .page-header {
      display: flex;
...
<div class="wrapper">
  ...
  <!-- MODALS (Nueva/Editar, Nueva Categoría, Detalle, Pacientes) -->
...
$(function(){
  ...
  dt = $('#tablaEnfermedades').DataTable({
    ...
    columns: [
      ...
      { data:null, orderable:false, className:'no-export',
        render:(r)=>`
          <button class="btn btn-sm btn-primary ver-pacientes" title="Ver Diagnósticos"><i class="fas fa-users"></i></button>
  ...
  $('#tablaEnfermedades').on('click','.ver-pacientes', function(){
    const d = dt.row($(this).closest('tr')).data();
    $('#lblEnfermedad').text(d.nombre);
    $('#listaPacientes').html('<li class="list-group-item">Cargando…</li>');
    $.getJSON('ajax/pacientes_por_enfermedad.php', { id_enfermedad:d.id, limit:5 }, function(r){
      const ul = $('#listaPacientes').empty();
      (r.items||[]).forEach(x=>{
        ul.append(`<li class="list-group-item d-flex justify-content-between align-items-center">
          ${x.paciente}<span class="badge badge-secondary">${x.fecha} · ${x.medico}</span></li>`);
      });
      if(!r.items || !r.items.length){ ul.html('<li class="list-group-item">Sin diagnósticos recientes.</li>'); }
    });
    $('#btnIrDiagnosticos').attr('href','historial_enfermedades.php?enfermedad_id='+d.id);
